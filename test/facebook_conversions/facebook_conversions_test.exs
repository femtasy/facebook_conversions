  ...
  describe "events/1" do
    test "send the list of events to Facebook" do
      ...
      Mox.expect(Tesla.MockAdapter, :call, 1, fn %Tesla.Env{url: url, body: body}, _opts ->
        ...
        expected_events = Jason.encode!(events)

        assert %Tesla.Multipart{
                 content_type_params: ["charset=utf-8"],
                 parts: [
                   %Tesla.Multipart.Part{body: ^expected_events, dispositions: [name: "data"]},
    ...
    test "sends the test even code" do
      event = %Event{
        event_name: "a facebook event",
        event_time: 23,
        action_source: "system_generated",
      ...
      Mox.expect(Tesla.MockAdapter, :call, 1, fn %Tesla.Env{url: url, body: body}, _opts ->
        ...
        assert %Tesla.Multipart{
                 ...
                 parts: [
                   ...
                   %Tesla.Multipart.Part{
                     body: "a test event code",
                     dispositions: [name: "test_event_code"]
                   }
                 ]
               } = body
      ...
      assert {:ok, :success} =
               FacebookConversions.send_events(events, test_event_code: "a test event code")
    end
  end
end
